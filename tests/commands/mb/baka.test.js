...
const mangaCommand = require('../../../src/commands/mangabaka/baka');
    ...
    describe('execute', () => {
        ...
        describe('with query parameter', () => {
            it('should search for manga and return title list when results found', async () => {
                ...
                expect(mockSearch).toHaveBeenCalledWith('test manga', 'mangabaka');
            ...
            it('should return error when no search results found', async () => {
                ...
                expect(mockSearch).toHaveBeenCalledWith('nonexistent manga', 'mangabaka');
        ...
        describe('with id parameter', () => {
            it('should get manga details by ID and return detailed embed', async () => {
                ...
                expect(checkID).toHaveBeenCalledWith('test-id-123', 'mangabaka');
                expect(mockGetTitleDetails).toHaveBeenCalledWith('test-id-123', 'mangabaka');
                //expect(mockGetTitleStats).toHaveBeenCalledWith('test-id-123', 'mangabaka'); // No stats endpoint yet
                expect(interaction.editReply).toHaveBeenCalledWith({
                    embeds: [expect.anything()],
                    files: [],
        ...
        describe('edge cases for code coverage', () => {
            ...
            it('should use ID when query is null but ID is provided (covers line 141)', async () => {
                ...
                // Mock specific parameter responses to test line 141: const mangaID = id || await parseURL(url, 'mangabaka');
                const interaction = {
                    type: InteractionType.ApplicationCommand,
                    commandName: 'baka',
                    options: {
                ...
                // Verify that the ID path was taken (line 141: const mangaID = id || await parseURL(url, 'mangabaka');)
                ...
                expect(checkID).toHaveBeenCalledWith('test-manga-id-123', 'mangabaka');
                expect(mockGetTitleDetails).toHaveBeenCalledWith('test-manga-id-123', 'mangabaka');
                //expect(mockGetTitleStats).toHaveBeenCalledWith('test-manga-id-123', 'mangabaka'); // No stats endpoint yet
            });
            ...
            it('should use URL when query and ID are null but URL is provided (covers parseURL in line 141)', async () => {
                ...
                const interaction = {
                    ...
                    options: {
                        getString: jest.fn((option) => {
                            ...
                            if (option === 'url') return 'https://mangabaka.dev/test-manga-id-456'; // URL is provided
                            return null;
                        })
                    },
                    user: { username: 'testUser' },
                ...
                // Verify that the URL path was taken (line 141: const mangaID = id || await parseURL(url, 'mangabaka');)
                expect(interaction.options.getString).toHaveBeenCalledWith('query');
                expect(interaction.options.getString).toHaveBeenCalledWith('id');
                expect(interaction.options.getString).toHaveBeenCalledWith('url');
                expect(parseURL).toHaveBeenCalledWith('https://mangabaka.dev/test-manga-id-456', 'mangabaka');
                expect(checkID).toHaveBeenCalledWith('test-manga-id-456', 'mangabaka');
                expect(mockGetTitleDetails).toHaveBeenCalledWith('test-manga-id-456', 'mangabaka');
                //expect(mockGetTitleStats).toHaveBeenCalledWith('test-manga-id-456', 'mangabaka'); // No stats endpoint yet
            });
            ...
            it('should handle invalid manga ID (covers line 142)', async () => {
                ...
                expect(checkID).toHaveBeenCalledWith('invalid-manga-id', 'mangabaka');
                expect(mockSendErrorEmbed).toHaveBeenCalledWith(
                    interaction,
                    expect.anything(), // translations
                    expect.anything(), // embed
            ...
            it('should handle null manga or stats data (covers line 145)', async () => {
                ...
                expect(checkID).toHaveBeenCalledWith('valid-id-but-no-data', 'mangabaka');
                expect(mockGetTitleDetails).toHaveBeenCalledWith('valid-id-but-no-data', 'mangabaka');
                //expect(mockGetTitleStats).toHaveBeenCalledWith('valid-id-but-no-data', 'mangabaka'); // No stats endpoint yet
                expect(mockSendErrorEmbed).toHaveBeenCalledWith(
                    interaction,
                    expect.anything(), // translations
                    expect.anything(), // embed
